...
namespace LogicLayer {
	...
	public abstract class ValidationViewModel : ViewModelBase, INotifyDataErrorInfo {
		...
		//TODO If a button gets clicked, the validation no longer works?!
		//TODO the ErrorsChanged Event should be Implemented seperately from the PropertyChanged, since it could get a huge performance impact, if CollectErrors always gets executed (Maybe Dictionary<propname, List<ValidationAttribute>> 
		...
		private void CollectErrors( string? propertyName = "" ) {
			...
			PropertyInfo? property = _VMType.GetProperty( propertyName );
			...
			foreach( Type? atttributeType in _ValidationAttributes ) {
				...
				object? methodReturnValue = methodInfo.Invoke( null, new object[] { property, atttributeType } );
